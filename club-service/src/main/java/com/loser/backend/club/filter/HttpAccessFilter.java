package com.loser.backend.club.filter;
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        for (; ; ) {
            ...
            if (headKey.equals(Constant.HEADER_USER))
                headMap.put(headKey,
                        Optional.ofNullable(JSON.parseObject(headVal))
                                .map(v -> v.getString(Constant.HEADER_USER_ID)).orElse(null));
            if (headKey.equals(Constant.ACEUP_USERNAME)){
                headMap.put(headKey,
                        Optional.ofNullable(headVal)
                                .map(v -> URLDecoder.decode(v, StandardCharsets.UTF_8)).orElse(null));
            }
        }
        ...
        responseEntity.put("response", new String(responseContent));
        long consumeTime = Duration.between(start, end).toMillis();
        log.info("#HttpAccessTrace, spend = {} ms, Request = {}, Response = {}", consumeTime, JSONObject.toJSONString(requestEntity), JSONObject.toJSONString(responseEntity));
